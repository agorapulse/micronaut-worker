...
You can run a single job from the command line using the `com.agorapulse.worker.runner.JobRunner` class as the main class. The arguments are the names of the jobs to run. Only the jobs named on the command line are scheduled; every other job is registered but never scheduled, even when enabled in the configuration. The application will run until all the named jobs are finished.
...
The runner enforces this by setting the `worker.scheduled-job-names` property to the job names from the command line before the application context starts. When that property is set (whether by the runner or directly in the configuration), only the listed jobs are scheduled. Setting it prevents the other jobs' `@QueueListener` consumers from ever starting their poll loop — a running infinite poll cannot be interrupted by disabling the job after the context has started.

[source,yaml]
.Scheduling only selected jobs
----
worker:
  scheduled-job-names:
    - sample-job
...
NOTE: Leave `worker.scheduled-job-names` empty (the default) to schedule every enabled job as usual.

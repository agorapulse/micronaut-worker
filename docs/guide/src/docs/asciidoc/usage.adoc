...
NOTE: All the jobs are also disabed for the `test` and `function` environment.

You can set the default queue type using the `worker.queue-type` configuration property. This can be very handy for local development to ensure your application is running against the local implementation.

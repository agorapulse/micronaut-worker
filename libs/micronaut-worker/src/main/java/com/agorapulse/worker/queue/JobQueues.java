...
public interface JobQueues {
    ...
    default <T> void readMessages(String queueName, int maxNumberOfMessages, Duration waitTime, Argument<T> argument, Consumer<T> action) {
        Flux.from(readMessages(queueName, maxNumberOfMessages, waitTime, argument)).map(QueueMessage::getMessage).subscribe(action);
    }
     ...
     * @param <T> the type of the message
     */
    <T> Publisher<QueueMessage<T>> readMessages(String queueName, int maxNumberOfMessages, Duration waitTime, Argument<T> argument);
